class CauchyArmLemmaCanvasUI extends CanvasUI {
	...
	stretchPolygon(polygon, a) {  // works great, but only on triangles
		// stretch polygon's edge AC with angle a-b-c 
	...
	rotate(a, b, c, factor = 1.1) {
		...
		const originalLengthBC = this.getDistance(b, c);
		const currentAngle = this.getAngle(a, b, c);

		// Determine the rotation angle (in radians)
		const delta = Math.acos(1 / factor);
		...
		const normalizedBCX = (rotatedBCX / magnitude) * originalLengthBC;
		const normalizedBCY = (rotatedBCY / magnitude) * originalLengthBC;
	...
	printDistance(triangle, a, str) {  
		// we assume the polygon is a triangle
		let b = (a + 1) % 3;
		let c = (b + 1) % 3;
